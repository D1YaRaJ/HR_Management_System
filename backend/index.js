...
//   // Destructure the arrays and remove from employeeData for main insert
//   const { qualifications, professionalExperiences, ...basicEmployeeData } = employeeData;

//   try {
//     // 1. Insert basic employee info (exclude qualifications and experiences)
//     const employeeQuery = `INSERT INTO employee_master (
//       EID, INITIAL, FIRSTNAME, MIDDLENAME, LASTNAME, DESIGNATION,
//       DOB, DATE_OF_JOIN, FTYPE, NATIONALITY, PHONE, EMAIL, CASTE,
//       DOORNO, CITY, STATE, PINCODE, GENDER, DID,
//       BIOMETRIC_CARD_NO, AADHAR, BANK_ACC, PAN,
//       FNAME, F_DOB, MNAME, M_DOB, PASSWORD
//     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

//     const employeeValues = [
//       basicEmployeeData.EID, basicEmployeeData.INITIAL, basicEmployeeData.FIRSTNAME,
//       basicEmployeeData.MIDDLENAME, basicEmployeeData.LASTNAME, basicEmployeeData.DESIGNATION,
//       basicEmployeeData.DOB, basicEmployeeData.DATE_OF_JOIN, basicEmployeeData.FTYPE,
//       basicEmployeeData.NATIONALITY, basicEmployeeData.PHONE, basicEmployeeData.EMAIL,
//       basicEmployeeData.CASTE, basicEmployeeData.DOORNO, basicEmployeeData.CITY,
//       basicEmployeeData.STATE, basicEmployeeData.PINCODE, basicEmployeeData.GENDER,
//       basicEmployeeData.DID,
//       basicEmployeeData.BIOMETRIC_CARD_NO, basicEmployeeData.AADHAR,
//       basicEmployeeData.BANK_ACC, basicEmployeeData.PAN,
//       basicEmployeeData.FNAME, basicEmployeeData.F_DOB, basicEmployeeData.MNAME,
//       basicEmployeeData.M_DOB, basicEmployeeData.EID // password as EID
//     ];

//     await pool.execute(employeeQuery, employeeValues);

//     // 2. Insert qualifications
//     if (qualifications && qualifications.length > 0) {
//       const qualQuery = `INSERT INTO employee_qualification (EID, INSTITUTION, PERCENTAGE, SPECIALIZATION, YOG) VALUES ?`;
//       const qualValues = qualifications.map(q => [
//         basicEmployeeData.EID, q.INSTITUTION, q.PERCENTAGE, q.SPECIALIZATION, q.YOG
//       ]);
//       await pool.query(qualQuery, [qualValues]);
//     }

//     // 3. Insert professional experiences
//     if (professionalExperiences && professionalExperiences.length > 0) {
//       const expQuery = `INSERT INTO employee_experience (EID, PROFEXP_DESIGNATION, PPROFEXP_FROM, PPROFEXP_TO) VALUES ?`;
//       const expValues = professionalExperiences.map(exp => [
//         basicEmployeeData.EID, exp.PROFEXP_DESIGNATION, exp.PPROFEXP_FROM, exp.PPROFEXP_TO
//       ]);
//       await pool.query(expQuery, [expValues]);
//     }
...
//   const { eid } = req.params;
//   try {
//     // 1. Employee main data
//     const [employeeRows] = await pool.execute(`SELECT * FROM employee_master WHERE EID = ?`, [eid]);
...
//     // 2. Qualifications
//     const [qualRows] = await pool.execute(`SELECT * FROM employee_qualification WHERE EID = ?`, [eid]);

//     // 3. Professional experiences
//     const [expRows] = await pool.execute(`SELECT * FROM employee_experience WHERE EID = ?`, [eid]);

//     // Combine all data into one object
//     const fullData = {
//       ...employee,
//       qualifications: qualRows,
//       professionalExperiences: expRows
//     };
...
//     const [employees] = await pool.execute(`SELECT * FROM employee_master ORDER BY EID`);

//     if (employees.length === 0) {
//       return res.json([]);
//     }

//     // Extract all EIDs to query qualifications and experiences in bulk
//     const eids = employees.map(emp => emp.EID);

//     // 2. Fetch all qualifications for these EIDs
//     const [qualifications] = await pool.query(
//       `SELECT * FROM employee_qualification WHERE EID IN (?)`,
//       [eids]
//     );

//     // 3. Fetch all experiences for these EIDs
//     const [experiences] = await pool.query(
//       `SELECT * FROM employee_experience WHERE EID IN (?)`,
//       [eids]
//     );

//     // 4. Map qualifications and experiences by EID for easy lookup
//     const qualificationsByEID = qualifications.reduce((acc, qual) => {
//       acc[qual.EID] = acc[qual.EID] || [];
//       acc[qual.EID].push(qual);
//       return acc;
//     }, {});

//     const experiencesByEID = experiences.reduce((acc, exp) => {
//       acc[exp.EID] = acc[exp.EID] || [];
//       acc[exp.EID].push(exp);
//       return acc;
//     }, {});

//     // 5. Attach qualifications and experiences to each employee object
//     const fullEmployees = employees.map(emp => ({
//       ...emp,
//       qualifications: qualificationsByEID[emp.EID] || [],
//       professionalExperiences: experiencesByEID[emp.EID] || []
//     }));

//     res.json(fullEmployees);
...
app.post("/apply-leave", async (req, res) => {
    try {
        ...
        // Check leave balance (optional - you can implement this later)
        // const balance = await checkLeaveBalance(EID, LTYPE, no_of_days);
  ...
  const { role, EID } = req.query;
    ...
    let sql = `
        SELECT l.leave_id, l.EID, e.FIRSTNAME, e.LASTNAME, l.LTYPE, 
               l.from_date, l.to_date, l.no_of_days, l.reason,CONCAT(h.FIRSTNAME, ' ', h.LASTNAME) AS handover_name, l.status, l.applied_date
        FROM emp_leave l
...
app.get('/api/payroll/download/:eid/:year/:month', async (req, res) => {
  ...
  try {
    ...
    // 2. Get payroll data - FIXED: Use template literal
    ...
    // Response headers - FIXED: Proper string concatenation
    ...
    // PDF Title - FIXED: Proper string concatenation
    doc.font('Helvetica-Bold').fontSize(20)
       .text('PAYSLIP', { align: 'center' })
       .moveDown();

    // Generated date - FIXED: Proper string concatenation
    doc.font('Helvetica').fontSize(10)
       .text(`Generated on: ${generatedDate}`, { align: 'right' })
       .moveDown();

    // Employee info - FIXED: Proper string concatenation
    ...
    // Salary details - FIXED: Proper string concatenation
